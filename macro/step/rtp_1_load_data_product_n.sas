...
****************************************************************************/
%macro rtp_1_load_data_product_n(mpMode=A,
	...
	/* Формирование таблицы на основании рассчитанных лагов + abt_promo  */
	/***************************************************************************************/
	proc fedsql sesref=casauto;
		create table casuser.abt_link{options replace = true} as
			select
				t1.PBO_LOCATION_ID,
				t1.PRODUCT_ID,
				t1.CHANNEL_CD,
				...
				t3.lag_week_std,
				t3.lag_year_std,
				t4.lag_halfyear_pct10,		 
				t4.lag_halfyear_pct90,		 
				t4.lag_month_pct10,
				t4.lag_month_pct90,
				t4.lag_qtr_pct10,	
				t4.lag_qtr_pct90,	
			...
			from
				casuser.lag_abt1 as t1
					...
					left join casuser.lag_abt3 as t4
	...
	/* Выгрузка вычисленных данных */
	proc casutil;
		save casdata='abt_link' incaslib='casuser' casout= 'abt_link.sashdat' outcaslib='mn_short';
		droptable casdata="lag_abt1" incaslib="casuser" quiet;
		droptable casdata="lag_abt2" incaslib="casuser" quiet;
		droptable casdata="lag_abt3" incaslib="casuser" quiet;
		droptable casdata='abt_promo' incaslib='casuser' quiet;
		droptable casdata='abt_link' incaslib='casuser' quiet;
	run;
	...
	proc fedsql sessref = casauto;
		create table casuser.abt5_ml{options replace = true} as
			select
				...
				t2.A_RDI
			from
				casuser.abt4_ml as t1 left join 
	...
	/* Загрузка лагов для присоединения */
	proc casutil;
		load casdata="abt_link.sashdat" incaslib="mn_short" casout="abt_link" outcaslib="casuser"; 
	run;
	...
	proc fedsql sessref = casauto;
		create table casuser.abt14_ml{options replace=true} as 
			...
			from
				...
				left join casuser.abt_link as t2
				on t1.PBO_LOCATION_ID = t2.PBO_LOCATION_ID
					AND t1.PRODUCT_ID = t2.PRODUCT_ID
					AND t1.CHANNEL_CD = t2.CHANNEL_CD
					AND t1.SALES_DT = t2.SALES_DT
	...
	%text_encoding(mpTable=casuser.abt14_ml, mpVariable=channel_cd);
...
%mend rtp_1_load_data_product_n;