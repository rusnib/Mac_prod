...
%let lmvTableFcst = MAX_CASL.FCST_UNITS_DEC;
%let lmvOutTablePostfix = DEC;
...
proc fedsql sessref=casauto;
	create table CASUSER.SALE_ACT {options replace=true} as
	select 
		  pbo.PBO_LOCATION_ID
		, pbo.product_id
		, pbo.SALES_DT
...
proc fedsql sessref=casauto;
	create table CASUSER.SALE_SAS_FCST {options replace=true} as
	select pbo.PBO_LOCATION_ID
		, pbo.product_id
		, pbo.SALES_DT
		, pbo.FINAL_FCST_UNITS_ML
		, pbo.FINAL_FCST_UNITS_REC_BPLM
		, pbo.FINAL_FCST_UNITS_REC_APLM
		, pr.price_net * pbo.FINAL_FCST_UNITS_ML       as FINAL_FCST_SALE_ML 
		, pr.price_net * pbo.FINAL_FCST_UNITS_REC_BPLM as FINAL_FCST_SALE_REC_BPLM
		, pr.price_net * pbo.FINAL_FCST_UNITS_REC_APLM as FINAL_FCST_SALE_REC_APLM
		, pr.price_net
/* 		, pr.price_net_curr */
	...
	left join MAX_CASL.KPI_prices 	as pr	
		on  pbo.product_id		= pr.product_id 
		and pbo.pbo_location_id	= pr.pbo_location_id 
		and pbo.sales_dt		= pr.period_dt
...
proc fedsql sessref=casauto;
	...
	select 
		...
		, sas.FINAL_FCST_UNITS_ML       as UNITS_SAS_FCST_ML 		
		, sas.FINAL_FCST_UNITS_REC_BPLM as UNITS_SAS_FCST_REC_BPLM
		, sas.FINAL_FCST_UNITS_REC_APLM as UNITS_SAS_FCST_REC_APLM
		
		, sas.FINAL_FCST_SALE_ML 		as SALE_SAS_FCST_ML 		
		, sas.FINAL_FCST_SALE_REC_BPLM	as SALE_SAS_FCST_REC_BPLM
		, sas.FINAL_FCST_SALE_REC_APLM	as SALE_SAS_FCST_REC_APLM

		, coalesce(act.SALE_ACT, 0) as SALE_ACT
		, coalesce(act.UNITS_ACT, 0) as UNITS_ACT

		, case 
			when sku.product_id is null or  loc.PBO_LOCATION_ID is null 
				then 0
			else 1
		  end as flag_filter			
...
/* 		, mcd.SALE_MCD_FCST */
	...
	from CASUSER.SALE_SAS_FCST  as sas

	left join CASUSER.SALE_ACT as act	
...
/* 		and sas.SALES_DT 		= act.SALES_DT */
	...
	left join CASUSER.PBO_LIST as loc
		on sas.PBO_LOCATION_ID = loc.PBO_LOCATION_ID
	
	left join CASUSER.PROD_LIST as sku
		on sas.product_id = sku.product_id
	;
quit;

proc casutil;
	droptable 
		casdata		= "SALE_FCST_VS_ACT_&lmvOutTablePostfix." 
		incaslib	= "MAX_CASL" 
		quiet         
	;                 
run;  

data MAX_CASL.SALE_FCST_VS_ACT_&lmvOutTablePostfix.;
	set CASUSER.SALE_FCST_VS_ACT;
run;
...
proc casutil;         
	promote           
		casdata		= "SALE_FCST_VS_ACT_&lmvOutTablePostfix." 
		incaslib	= "MAX_CASL" 
		casout		= "SALE_FCST_VS_ACT_&lmvOutTablePostfix."  
		outcaslib	= "MAX_CASL"
	;                 
run;  
...
proc fedsql sessref=casauto;
	...
	select *
		...
		, (SALE_SAS_FCST_ML 	 	- SALE_ACT) AS SALE_SAS_ERR_ML 		
		, (SALE_SAS_FCST_REC_BPLM 	- SALE_ACT) AS SALE_SAS_ERR_REC_BPLM
		, (SALE_SAS_FCST_REC_APLM 	- SALE_ACT) AS SALE_SAS_ERR_REC_APLM
		
		, (UNITS_SAS_FCST_ML 	 	- UNITS_ACT) AS UNITS_SAS_ERR_ML 		
		, (UNITS_SAS_FCST_REC_BPLM 	- UNITS_ACT) AS UNITS_SAS_ERR_REC_BPLM
		, (UNITS_SAS_FCST_REC_APLM 	- UNITS_ACT) AS UNITS_SAS_ERR_REC_APLM
		
		, abs(SALE_SAS_FCST_ML 	 		- SALE_ACT) AS SALE_SAS_ABSERR_ML 		
		, abs(SALE_SAS_FCST_REC_BPLM 	- SALE_ACT) AS SALE_SAS_ABSERR_REC_BPLM
		, abs(SALE_SAS_FCST_REC_APLM 	- SALE_ACT) AS SALE_SAS_ABSERR_REC_APLM

		, abs(UNITS_SAS_FCST_ML 	 	- UNITS_ACT) AS UNITS_SAS_ABSERR_ML 		
		, abs(UNITS_SAS_FCST_REC_BPLM 	- UNITS_ACT) AS UNITS_SAS_ABSERR_REC_BPLM
		, abs(UNITS_SAS_FCST_REC_APLM 	- UNITS_ACT) AS UNITS_SAS_ABSERR_REC_APLM
...
proc fedsql sessref=casauto;
	...
	select month_dt
		...
		, sum(SALE_SAS_ABSERR_ML 		) / sum(SALE_ACT ) as WAPE_SAS_SALE_ML 		
		, sum(SALE_SAS_ABSERR_REC_BPLM 	) / sum(SALE_ACT ) as WAPE_SAS_SALE_REC_BPLM
		, sum(SALE_SAS_ABSERR_REC_APLM 	) / sum(SALE_ACT ) as WAPE_SAS_SALE_REC_APLM
		
		, sum(UNITS_SAS_ABSERR_ML 		) / sum(UNITS_ACT ) as WAPE_SAS_UNITS_ML 		
		, sum(UNITS_SAS_ABSERR_REC_BPLM ) / sum(UNITS_ACT ) as WAPE_SAS_UNITS_REC_BPLM
		, sum(UNITS_SAS_ABSERR_REC_APLM ) / sum(UNITS_ACT ) as WAPE_SAS_UNITS_REC_APLM
		
/* 		, sum(SALE_mcd_abserr ) / sum(SALE_act ) as WAPE_MCD */

		, sum(SALE_SAS_ERR_ML 		    ) / sum(SALE_ACT ) as BIAS_SAS_SALE_ML 		
		, sum(SALE_SAS_ERR_REC_BPLM    	) / sum(SALE_ACT ) as BIAS_SAS_SALE_REC_BPLM
		, sum(SALE_SAS_ERR_REC_APLM    	) / sum(SALE_ACT ) as BIAS_SAS_SALE_REC_APLM
		
		, sum(UNITS_SAS_ERR_ML 		 ) / sum(UNITS_ACT ) as BIAS_SAS_UNITS_ML 		
		, sum(UNITS_SAS_ERR_REC_BPLM ) / sum(UNITS_ACT ) as BIAS_SAS_UNITS_REC_BPLM
		, sum(UNITS_SAS_ERR_REC_APLM ) / sum(UNITS_ACT ) as BIAS_SAS_UNITS_REC_APLM
		
/* 		, sum(SALE_mcd_err    ) / sum(SALE_act ) as BIAS_MCD */

		, sum(SALE_ACT ) as SUM_SALE_ACT

		, sum(SALE_SAS_FCST_ML 		) as SUM_SALE_SAS_FCST_ML 		
		, sum(SALE_SAS_FCST_REC_BPLM) as SUM_SALE_SAS_FCST_REC_BPLM
		, sum(SALE_SAS_FCST_REC_APLM) as SUM_SALE_SAS_FCST_REC_APLM
		
		, sum(UNITS_ACT ) as SUM_UNITS_ACT

		, sum(UNITS_SAS_FCST_ML 	 ) as SUM_UNITS_SAS_FCST_ML 		
		, sum(UNITS_SAS_FCST_REC_BPLM) as SUM_UNITS_SAS_FCST_REC_BPLM
		, sum(UNITS_SAS_FCST_REC_APLM) as SUM_UNITS_SAS_FCST_REC_APLM
		
/* 		, sum(SALE_mcd_fcst) as sum_SALE_mcd_fcst */
	
		, sum(SALE_SAS_ABSERR_ML 		 ) as SUM_SALE_SAS_ABSERR_ML 		
		, sum(SALE_SAS_ABSERR_REC_BPLM 	 ) as SUM_SALE_SAS_ABSERR_REC_BPLM 	
		, sum(SALE_SAS_ABSERR_REC_APLM 	 ) as SUM_SALE_SAS_ABSERR_REC_APLM 
		
		, sum(UNITS_SAS_ABSERR_ML 		 ) as SUM_UNITS_SAS_ABSERR_ML 		
		, sum(UNITS_SAS_ABSERR_REC_BPLM  ) as SUM_UNITS_SAS_ABSERR_REC_BPLM 	
		, sum(UNITS_SAS_ABSERR_REC_APLM  ) as SUM_UNITS_SAS_ABSERR_REC_APLM 
		
/* 		, sum(SALE_mcd_abserr ) as sum_SALE_mcd_abserr */
		
		, sum(SALE_SAS_ERR_ML 		 ) as SUM_SALE_SAS_ERR_ML 		
		, sum(SALE_SAS_ERR_REC_BPLM  ) as SUM_SALE_SAS_ERR_REC_BPLM 	
		, sum(SALE_SAS_ERR_REC_APLM  ) as SUM_SALE_SAS_ERR_REC_APLM 
		
		, sum(UNITS_SAS_ERR_ML 		 ) as SUM_UNITS_SAS_ERR_ML 		
		, sum(UNITS_SAS_ERR_REC_BPLM ) as SUM_UNITS_SAS_ERR_REC_BPLM 	
		, sum(UNITS_SAS_ERR_REC_APLM ) as SUM_UNITS_SAS_ERR_REC_APLM 
...
data WORK.KPI_MONTH_&lmvOutTablePostfix.;
	...
	format
		month_dt 			date9.
		WAPE_SAS_SALE_ML 		PERCENTN8.2
		WAPE_SAS_SALE_REC_BPLM	PERCENTN8.2
		WAPE_SAS_SALE_REC_APLM	PERCENTN8.2
		BIAS_SAS_SALE_ML 		PERCENTN8.2
		BIAS_SAS_SALE_REC_BPLM	PERCENTN8.2
		BIAS_SAS_SALE_REC_APLM	PERCENTN8.2
		WAPE_SAS_UNITS_ML 		PERCENTN8.2
		WAPE_SAS_UNITS_REC_BPLM	PERCENTN8.2
		WAPE_SAS_UNITS_REC_APLM	PERCENTN8.2
		BIAS_SAS_UNITS_ML 		PERCENTN8.2
		BIAS_SAS_UNITS_REC_BPLM	PERCENTN8.2
		BIAS_SAS_UNITS_REC_APLM	PERCENTN8.2
		SUM_SALE_ACT 					COMMAX15.
		SUM_SALE_SAS_FCST_ML 	 		COMMAX15.
		SUM_SALE_SAS_FCST_REC_BPLM 		COMMAX15.
		SUM_SALE_SAS_FCST_REC_APLM 		COMMAX15.
		SUM_SALE_SAS_ABSERR_ML 	 		COMMAX15.
		SUM_SALE_SAS_ABSERR_REC_BPLM 	COMMAX15.
		SUM_SALE_SAS_ABSERR_REC_APLM	COMMAX15.
		SUM_SALE_SAS_ERR_ML 	 		COMMAX15.
		SUM_SALE_SAS_ERR_REC_BPLM 		COMMAX15.
		SUM_SALE_SAS_ERR_REC_APLM 		COMMAX15.
		SUM_UNITS_ACT 					COMMAX15.
		SUM_UNITS_SAS_FCST_ML 	 		COMMAX15.
		SUM_UNITS_SAS_FCST_REC_BPLM 	COMMAX15.
		SUM_UNITS_SAS_FCST_REC_APLM 	COMMAX15.
		SUM_UNITS_SAS_ABSERR_ML 	 	COMMAX15.
		SUM_UNITS_SAS_ABSERR_REC_BPLM 	COMMAX15.
		SUM_UNITS_SAS_ABSERR_REC_APLM	COMMAX15.
		SUM_UNITS_SAS_ERR_ML 	 		COMMAX15.
		SUM_UNITS_SAS_ERR_REC_BPLM 		COMMAX15.
		SUM_UNITS_SAS_ERR_REC_APLM 		COMMAX15.
		/*
...
ods excel file="&common_path./KPI_SALE_&lmvOutTablePostfix..xlsx"  style=statistical;

ods excel options(sheet_interval = 'none' sheet_name = "KPI_SALE_WEEK"	);
proc print data = WORK.KPI_WEEK_&lmvOutTablePostfix. 	label; run;

ods excel options(sheet_interval = 'proc' sheet_name = "KPI_SALE_MONTH"	);
