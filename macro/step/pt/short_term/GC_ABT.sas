...
%let inlib=IA;
%let sas_history_end_dt = '18may2021'd;
%let history_end_dt = date '2021-05-18';

%let start_date = '19may2021'd;
%let end_date = '19aug2021'd;
%let fcst_start_dt = date '2021-05-19';


/* 1. Timeseries MA[7] */
/* proc casutil; */
/*   load data=&inlib..ia_pbo_sales_history casout='ia_pbo_sales_history' outcaslib='CASUSER' replace; */
/* run; */
...
/* 2. Comp PBO */
/* proc casutil; */
/* 	load data=ETL_STG.comp_geo_for_nov2020 casout='comp_geo_for_nov2020' outcaslib='CASUSER' replace; */
/* run; */
...
/*            INNER JOIN CASUSER.comp_geo_for_nov2020 t3  */
...
/* 10.  */
PROC FEDSQL sessref=casauto;
   CREATE TABLE CASUSER.PRE_COVID_WOY_DOW{options replace=true} AS 
   SELECT t1.PBO_LOCATION_ID, 
          t1.CHANNEL_CD, 
...
/* 11.  */
PROC SORT
	DATA=CASUSER.PRE_COVID_WOY_DOW(KEEP=SALES_DT RECEIPT_QTY CHANNEL_CD PBO_LOCATION_ID)
	OUT=WORK.TMP0TempTableInput
	;
...
PROC EXPAND DATA=WORK.TMP0TempTableInput
	OUT=CASUSER.MA364_TIMESERIES(LABEL="Modified Time Series data for CASUSER.PRE_COVID_WOY_DOW")
	ALIGN = BEGINNING
	METHOD = SPLINE(NOTAKNOT, NOTAKNOT) 
	OBSERVED = (BEGINNING, BEGINNING) 
;
...
/* 12.  */
PROC FEDSQL sessref=casauto;
   CREATE TABLE CASUSER.MA364_WITH_WOY_DOW{options replace=true} AS 
   SELECT t1.PBO_LOCATION_ID, 
          t1.CHANNEL_CD, 
      ...
      FROM CASUSER.PRE_COVID_WOY_DOW t1, CASUSER.MA364_TIMESERIES t2
...
PROC FEDSQL sessref=casauto;
   ...
   SELECT t1.PBO_LOCATION_ID, 
      ...
      FROM CASUSER.MA364_WITH_WOY_DOW t1
...
/* 23.  */
/* proc casutil; */
/*   load data=&inlib..MCD_COVID_PATTERN_DAY casout='MCD_COVID_PATTERN_DAY' outcaslib='CASUSER' replace; */
/* run; */
...
/* 		  t1.CHANNEL_CD,  */
	  ...
	  INNER JOIN CASUSER.IA_PROMO_X_PBO_LEAF t2
...
/* 25.  */
PROC FEDSQL sessref=casauto;
   CREATE TABLE MAX_CASL.DM_TRAIN_TRP_GC{options replace=true} AS 
   SELECT t1.CHANNEL_CD, 
          t1.PBO_LOCATION_ID, 
          t1.SALES_DT, 
          t1.COVID_pattern, 
...
proc casutil incaslib='MAX_CASL' outcaslib='MAX_CASL';
