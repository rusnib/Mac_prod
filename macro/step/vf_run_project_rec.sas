...
*     %vf_run_project_rec(mpProjectName=pbo_sales_v2);
* 
...
****************************************************************************/
%macro vf_run_project_rec(mpProjectName=, mpLock=Y);
	option nomprint nosymbolgen mlogic;
	...
	data work.list_projects;
	  set respjson.items;
	run;

	/* Извлечение ID для VF-проекта по его имени */
	%let lmvVfPboName = &mpProjectName.;
	%let lmvProjectId = %vf_get_project_id_by_name(mpName=&lmvVfPboName., mpProjList=work.list_projects);
