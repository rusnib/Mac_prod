	...
	%include '/opt/sas/mcd_config/config/rtp_load_data_to_caslib.sas';
	%rtp_load_data_to_caslib(mpWorkCaslib=mn_short);
	

	%let mpInCaslib = mn_short;
	...
	/* Фильтруем Lifecycle = N, меняем типы дат и обрабатываем пропуски в ресторанах */
	proc fedsql sessref=casauto;
		create table casuser.n_product_chain{options replace=true} as
			select
				...
				coalesce(PREDECESSOR_DIM2_ID, SUCCESSOR_DIM2_ID) as PREDECESSOR_DIM2_ID,
				...
				SCALE_FACTOR_PCT
			from
				mn_short.product_chain_enh
			where
				LIFECYCLE_CD = 'N' and successor_dim2_id is not missing

		union

			select
				t1.LIFECYCLE_CD,
				coalesce(t1.PREDECESSOR_PRODUCT_ID, t1.SUCCESSOR_PRODUCT_ID) as PREDECESSOR_PRODUCT_ID,
				t2.successor_dim2_id as PREDECESSOR_DIM2_ID,
				t1.SUCCESSOR_PRODUCT_ID,
				t2.successor_dim2_id,
				datepart(t1.SUCCESSOR_START_DT) as SUCCESSOR_START_DT,
				coalesce(datepart(t1.PREDECESSOR_END_DT), date %tslit(&VF_FC_AGG_END_DT)) as PREDECESSOR_END_DT,
				t1.SCALE_FACTOR_PCT
			from (
				select 
					* 
				from 
					mn_short.product_chain_enh
				where 
					LIFECYCLE_CD = 'N' and successor_dim2_id is missing
			) as t1,
            (select distinct successor_dim2_id from mn_short.product_chain_enh 
				where LIFECYCLE_CD = 'N' and successor_dim2_id is not missing) as t2
		;
	quit;
