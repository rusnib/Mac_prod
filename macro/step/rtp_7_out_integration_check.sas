...
/*4.1 Units*/
	...
	create table &lmvOutLibrefPmixSt..&lmvOutTabNamePmixSt.{options replace=true} as
		select distinct
			...
			as BASE_FCST_UNITS/*– базовый прогноз (заполняется, если в этом разрезе 
							товар-ПБО-день не было ни одной промо-акции, =0 иначе)*/,
			case when t1.promo=1 then t1.FF else 0 end
			as PROMO_FCST_UNITS /*– прогноз промо (заполняется, если в этом разрезе 
							товар-ПБО-день была одна и более промо-акций, =0 иначе)*/,
			t1.FF as FINAL_FCST_UNITS /*– сумма прогноза базового и промо*/,
			t1.FF as OVERRIDED_FCST_UNITS /*– сумма прогноза базового и промо (чем отличается от предыдущей строки?)*/,
			1 as OVERRIDE_TRIGGER/*– тригер оверрайда, по умолчанию значение 1*/,
			case when promo=0 then t1.ff*t2.net_price_amt else 0 end
			as BASE_FCST_SALE /*– базовый прогноз в РУБ (для пересчета штук в рубли используется net-цена? 
			...
			as PROMO_FCST_SALE /*– промо прогноз в РУБ (заполняется, если в этом разрезе товар-ПБО-день есть одна и более промо-акций)*/,
			t1.ff*t2.net_price_amt as FINAL_FCST_SALE  /*– суммарный прогноз в РУБ*/,
			t1.ff*t2.net_price_amt as OVERRIDED_FCST_SALE  /*– Прогноз с учетом оверрйда РУБ (считается в ETL путем умножения средней цены на прогноз с учетом оверрайдов).*/,
			t2.net_price_amt as AVG_PRICE /*– средняя цена. Считается в ETL как отношение прогноз в руб/прогноз в шт в разрезе СКЮ/ПБО*/
			from casuser.fcst_reconciled t1 left join casuser.ts_price_fact t2 on
...
/*4.2 GC:*/
	...
	create table &lmvOutLibrefGcSt..&lmvOutTabNameGcSt.{options replace=true} as
		select distinct
			...
			'RUR' as CURRENCY /*– Валюта, значение по умолчанию RUR*/,
			/*'CORP' as ORG /*– Организация, значение по умолчанию CORP,*/
			FF as BASE_FCST_GC /*– базовый прогноз */,
			0 as PROMO_FCST_GC /*– прогноз промо*/,
			FF as FINAL_FCST_GC /*– сумма прогноза базового и промо*/,
			FF as OVERRIDED_FCST_GC /*– сумма прогноза базового и промо с учетом оверрайдов*/,
			1 as OVERRIDE_TRIGGER /*– тригер оверрайда, по умолчанию значение 1*/
	...
	*/
	...
	create table &lmvOutLibrefUptSt..&lmvOutTabNameUptSt.{options replace=true} as
		...
		case when t2.BASE_FCST_GC is not null and abs(t2.BASE_FCST_GC)> 1e-5 
		   ...
		   as BASE_FCST_UPT /*– базовый прогноз, = Прогноз в ШТ(Товар, ПБО, день)/Прогноз GC(ПБО, день)*1000,
						если в разрезе Товар-ПБО-день нет ни одной промо-акции, =0 иначе.*/,
		case when t2.BASE_FCST_GC is not null and abs(t2.BASE_FCST_GC)> 1e-5
		   then t1.PROMO_FCST_UNITS/t2.BASE_FCST_GC*1000 
		   else 0
		   end
		   as PROMO_FCST_UPT /*– прогноз промо, = Прогноз в ШТ(Товар, ПБО, день)/Прогноз GC(ПБО, день)*1000, 
	...
	proc fedsql sessref=casauto;
		create table &lmvOutLibrefPmixLt..&lmvOutTabNamePmixLt.{options replace=true} as
			...
			sum(case when promo=0 then t1.FF else 0 end) 
			   as BASE_FCST_UNITS /*– базовый прогноз*/,
			sum(case when promo=1 then t1.FF else 0 end)
			   as PROMO_FCST_UNITS /*– прогноз промо*/,
			sum(FF) as FINAL_FCST_UNITS /*– сумма прогноза базового и промо*/,
			sum(FF) as OVERRIDED_FCST_UNITS /*– сумма прогноза базового и промо*/,
			1 as OVERRIDE_TRIGGER /*– тригер оверрайда, по умолчанию значение 1*/,
			sum(case when promo=0 then t1.ff*t2.net_price_amt else 0 end)
			   as BASE_FCST_SALE /*– базовый прогноз в РУБ*/,
			sum(case when promo=1 then t1.ff*t2.net_price_amt else 0 end)
			   as PROMO_FCST_SALE /*– промо прогноз в РУБ*/,
			sum(t1.ff*t2.net_price_amt)
			   as FINAL_FCST_SALE /*– суммарный прогноз в РУБ*/,
			sum(t1.ff*t2.net_price_amt)
			   as OVERRIDED_FCST_SALE /*– Прогноз с учетом оверрйда РУБ (считается в ETL путем умножения средней цены на прогноз с учетом оверрайдов).*/,
	...
	proc fedsql sessref=casauto;
		create table &lmvOutLibrefUptLt..&lmvOutTabNameUptLt.{options replace=true} as
			...
			case when t2.BASE_FCST_GC is not null and abs(t2.BASE_FCST_GC)>1e-5 
			   ...
			   as FINAL_FCST_UPT /*– суммарный прогноз*/,
			case when t2.BASE_FCST_GC is not null and abs(t2.BASE_FCST_GC)>1e-5 
			   then t1.FINAL_FCST_UNITS/t2.BASE_FCST_GC*1000 
			   else 0
			   end
			   as OVERRIDED_FCST_UPT /*– суммарный прогноз (с учетом логики сохранения оверрайдов)*/,
