/********************************************************
* НАЗНАЧЕНИЕ:
*    Макрос для выгрузки и форматирования данных в PostgreSQL
*
* ПАРАМЕТРЫ:
...
*********************************************************
* ОПИСАНИЕ АЛГОРИТМА:
*   1) Проверка существования ресурса mpResource в БД
*   2) Определение типа (FULL или WINDOW) и соответствующих параметров загрузки для ресурса
*   3) 
*   4) Получение списка переменных (колонок) из таблицы ресурса, имеющих формат даты
*   5) Закрытие(C) всех доступных(A) выгрузок
*   6) Добавление статуса текущей выгрузки 
*   7) Получение номера последней версии выгрузки
*   8) Формирование таблицы ресурса
*      8.1) Если тип выгрузки FULL, то таблица IA.IA_ресурс выгружается полностью
*      8.2) Если тип выгрузки WINDOW - только выбранный период
*      8.3) Для переменных формата даты происходит заполнение данных по соответствующему формату
*   9) Добавление сформированных данных в целевую таблицу ресурса в PostgreSQL
*   10) Обновление статуса ресурса на доступный(A).
*       В случае возникновения ошибок ставится соответствующая метка(E)
*********************************************************/
