%macro fmk_load_etl_ia_n(mpResource=);
	...
	%if &lmvHashDiffsLength gt 0 %then %do;
		...
		/* NEW BLOCK */
		proc append base=work.&lmvResource. data=work.&lmvResource._chkd_dlt (drop=pk_hash value_hash) force;
		run;
		
		%cmn_load_etl_ia_artefacts(mpMode=delta
								,mpTargetTableNm=&lmvResource.
								,mpInputTableNm=work.&lmvResource
								);
	%end;
	%else %do;
		%cmn_load_etl_ia_artefacts(mpMode=delta
